...
**Keywords.** Mountain pine beetle, forest disturbance, insect outbreaks, Landsat time series, refugia, British Columbia

**Data availability statement.** Every dataset used was public. Beetle disturbance was classified from Landsat Collection 2 Level-2 surface reflectance, stand structure came from the provincial Vegetation Resources Inventory, terrain from the Natural Resources Canada High Resolution Digital Elevation Model [@nrcan2017] and wind from Environment and Climate Change Canada hourly station records. All derived data and the complete analysis code that reproduce every number, table and figure in this article are at <https://github.com/seamusrobertmurphy/beetle-topography-and-wind-study> and will be deposited in the Dryad Digital Repository, with a DOI, with the revised manuscript.

**Conflict of interest statement.** The author has no conflict of interest to declare. There is no dispute over the ownership of the data presented, and every contribution has been attributed by acknowledgement.

**Author contributions.** Seamus Murphy took every CRediT role, conceptualization, methodology, software, validation, formal analysis, investigation, data curation, visualization, project administration, writing of the original draft, and review and editing.
...
## The abstract is the last thing measured, because Agricultural and Forest Entomology asks
## for "a numbered abstract of up to 250 words" and an abstract over the cap is returned.
...
2. All three were tested across `r fmt(AREA_HA)` ha of the Selkirk Mountains, British Columbia, using `r EP_EPOCHS` sixteen-day Landsat epochs over `r spellnum(EP_YEARS)` outbreak years with annualized forest inventory, geomorphometric variables and a terrain-resolved MicroMet wind field from hourly station records.
3. Wind disruption was supported conditionally, in that attack fell where a thin stand and strong flight-period wind coincided, the density-by-wind interaction `r be1(EINT[1])` for stem density (`r pe1T(EINT[1])`) and `r be1(EINT[2])` for standing volume (`r pe1T(EINT[2])`), both remaining after within-season contagion entered the model.
4. Terrain acted differently. Leeward ground had more attack as a main effect, `r b3("wind_shelter")`, independent of stand density, and open gentle ground more still, sky view `r b3("sky_view")`, which is the pattern expected if wind-borne beetles, which land without discriminating hosts, settle where the flow slows. Attack responded to host size as a threshold, peaking at `r qb("attack_pc", "25-30")` per cent in the 25 to 30 cm quadratic mean diameter class.
5. The shade hypothesis failed, since north-facing slopes showed more attack (`r b3("northness")`). Refugia are therefore proposed to lie on warm, windward ground in thin stands during windy flight periods, and the terrain ruggedness signal of the companion study resolves into shelter and openness.
