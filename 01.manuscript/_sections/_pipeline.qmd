...
## SUPERSEDED 2026-09-02 by chunk 24b. Kept as the record of the 1000-plot derivation, whose
## forest mask admitted open water; its outputs are archived as *-derived-plots.
...
## SUPERSEDED 2026-09-02 by chunk 24b, which trains on the ground plots. Kept as the record.
...
```{r}
#| label: pipeline-24b-fieldplot-classifier
#| eval: false
#| echo: false
#| output: false
#| warning: false
...
## The classifier of record since 2026-09-02. The 28 ground plots of the parent study,
## labelled low, moderate and high by tertiles of killed pine basal area, plus derived
## undisturbed plots sampled where the imagery shows no drop, train the same SVM on the
## same predictors, and the annual maps are predicted over the study perimeter only,
## with water masked. Outputs go to 02.inputs/beetle-fieldplots, never to beetle/.
suppressPackageStartupMessages({library(terra); library(sf); library(e1071); library(readxl)})
set.seed(42)
ROOT <- here::here("02.inputs", "beetle")
IN   <- file.path(ROOT, "ndmi-darkwoods"); SA <- file.path(ROOT, "study-area")
LOC  <- file.path(ROOT, "plot-locations"); OUT <- file.path(ROOT, "red-stage-darkwoods")
N_UNDIST <- 10L; MIN_SEP <- 120; STABLE <- -0.05
COST <- 0.947; GAMMA <- 0.10; PRED <- c("dndvi","dnbr","dtcw")
OY <- c(2006:2011, 2013, 2014)
rd  <- function(nm, y) rast(file.path(IN, sprintf("%s_%d.tif", nm, y)))
dif <- function(nm, y) { r <- rd(nm, y) - rd(nm, 2005); names(r) <- paste0("d", nm); r }

## ---- field plots, severity from the measurement alone ------------------------------
fp <- as.data.frame(read_excel(file.path(LOC, "2.1.darkwoods_beetle_ground_plots_ndmi.xlsx")))
br <- quantile(fp$pi_mpb_killed, c(1/3, 2/3))
fp$severity <- as.character(cut(fp$pi_mpb_killed, c(-Inf, br, Inf), labels = c("low","moderate","high")))
cat(sprintf("field plots %d; killed basal area tertiles %.3f and %.3f m2/ha; %s\n",
            nrow(fp), br[1], br[2], paste(names(table(fp$severity)), table(fp$severity), collapse = " ")))

## ---- undisturbed plots, derived where the imagery never dropped --------------------
per <- st_transform(st_read(file.path(SA, "study_perimeter.gpkg"), quiet = TRUE), 32611)
wat <- st_transform(st_read(file.path(SA, "basemap_water.geojson"), quiet = TRUE), 32611)
base <- rd("ndmi", 2005)
bb <- ext(vect(st_buffer(per, 2000)))
base_c <- crop(base, bb)
dmin <- min(rast(lapply(OY, function(y) crop(rd("ndmi", y), bb) - base_c)))
forest <- base_c > 0.20
forest <- mask(forest, vect(wat), inverse = TRUE)           # water is not forest
cand <- mask(ifel(forest == 1 & dmin > STABLE, 1, NA), vect(per))
sp <- spatSample(cand, size = N_UNDIST * 60, method = "random", as.points = TRUE, na.rm = TRUE)
xy <- crds(sp); keep <- integer(0)
for (i in seq_len(nrow(xy))) {
  if (!length(keep) || min(sqrt((xy[keep,1]-xy[i,1])^2 + (xy[keep,2]-xy[i,2])^2)) >= MIN_SEP) keep <- c(keep, i)
  if (length(keep) == N_UNDIST) break
}
ud <- data.frame(plot = NA_integer_, pi_mpb_killed = NA_real_, easting = xy[keep,1], northing = xy[keep,2],
                 severity = "unaffected", source = "derived")
fp2 <- data.frame(plot = fp$plot, pi_mpb_killed = fp$pi_mpb_killed, easting = fp$easting, northing = fp$northing,
                  severity = fp$severity, source = "field")
pl <- rbind(fp2, ud)
pv <- vect(pl, geom = c("easting","northing"), crs = "EPSG:32611")

## worst year from the imagery, as for every other plot set in this study
dst <- rast(lapply(OY, function(y) rd("ndmi", y) - base)); names(dst) <- OY
ex  <- terra::extract(dst, pv, ID = FALSE)
pl$worst_year <- OY[apply(ex, 1, which.min)]; pl$dndmi_min <- apply(ex, 1, min)
pl$on_water <- lengths(st_intersects(st_as_sf(pv), wat)) > 0
stopifnot(!any(pl$on_water))
write.csv(pl, file.path(LOC, "fieldplots_with_undisturbed.csv"), row.names = FALSE)
cat(sprintf("plot set written: %d plots, %s\n", nrow(pl), paste(names(table(pl$severity)), table(pl$severity), collapse = " ")))

## ---- the same SVM ----------------------------------------------------------------
X <- matrix(NA_real_, nrow(pl), length(PRED), dimnames = list(NULL, PRED))
for (y in OY) {
  k <- which(pl$worst_year == y); if (!length(k)) next
  X[k, ] <- as.matrix(terra::extract(rast(list(dif("ndvi", y), dif("nbr", y), dif("tcw", y))), pv[k], ID = FALSE))
}
tr <- data.frame(X, cls = factor(ifelse(pl$severity %in% c("moderate","high"), "modhigh", "other"), levels = c("other","modhigh")))
tr <- tr[stats::complete.cases(tr), ]
cat(sprintf("training pool %d, %s\n", nrow(tr), paste(names(table(tr$cls)), table(tr$cls), collapse = " / ")))
f <- as.formula(paste("cls ~", paste(PRED, collapse = " + ")))
## leave-one-out, because 38 plots do not support a held-out quarter
loo <- factor(sapply(seq_len(nrow(tr)), function(i)
  as.character(predict(svm(f, data = tr[-i, ], kernel = "radial", cost = COST, gamma = GAMMA), tr[i, ]))),
  levels = levels(tr$cls))
cm <- table(predicted = loo, observed = tr$cls); print(cm)
oa <- sum(diag(cm))/sum(cm); pe <- sum(rowSums(cm)*colSums(cm))/sum(cm)^2
cat(sprintf("leave-one-out accuracy %.3f  kappa %.3f\n", oa, (oa-pe)/(1-pe)))
write.csv(data.frame(n = nrow(tr), accuracy = oa, kappa = (oa-pe)/(1-pe),
                     tp = cm["modhigh","modhigh"], fp = cm["modhigh","other"], fn = cm["other","modhigh"], tn = cm["other","other"]),
          file.path(OUT, "classifier_loo.csv"), row.names = FALSE)

## ---- annual maps over the perimeter, water masked, no union -----------------------
mfull <- svm(f, data = tr, kernel = "radial", cost = COST, gamma = GAMMA)
watv <- vect(wat); res <- list()
for (y in OY) {
  st <- crop(rast(list(dif("ndvi", y), dif("nbr", y), dif("tcw", y))), bb)
  st <- mask(st, watv, inverse = TRUE)
  cl <- terra::predict(st, mfull, na.rm = TRUE,
        filename = file.path(OUT, sprintf("modhigh_%d.tif", y)), overwrite = TRUE,
        wopt = list(datatype = "INT1U", gdal = "COMPRESS=DEFLATE"))
  fq <- freq(cl); ct <- setNames(fq$count, as.character(fq$value)); tot <- sum(fq$count); a <- sum(ct["modhigh"], na.rm = TRUE)
  res[[length(res)+1]] <- data.frame(year = y, valid = tot, modhigh_cells = a, modhigh_pc = 100*a/tot, modhigh_ha = a*900/1e4)
  cat(sprintf("%d  moderate-to-high %6.2f%%\n", y, 100*a/tot))
}
write.csv(do.call(rbind, res), file.path(OUT, "modhigh_by_year.csv"), row.names = FALSE)
cat("annual maps written to", OUT, "\n")
```
...
## SUPERSEDED 2026-09-02. This chunk fitted on epoch_model_table.csv, the static-composite
## frame, so its all-hours row disagreed with the reported model E1, which is fitted on
## the annualized frame. The refit now runs in _preamble.qmd as fw_fit(), on the same frame
## as E1, and its CSV is no longer read. The chunk is kept for the record of how the
...
## Annualized stand structure for the modelling tables.
##
...
cat("\nannual means by year, annualized:\n")
...
## Refit every model on annualized stand structure, and report what changed.
##
...
## composite against the annualized snapshots. Whatever the interaction does, it is
...
res <- bind_rows(
  ...
  fit_epoch(file.path(MD, "epoch_model_table_vri.csv"), "annualized VRI"))
