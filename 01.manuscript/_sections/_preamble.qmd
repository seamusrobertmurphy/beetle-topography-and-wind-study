...
## A variant document may set BEETLE_ROOT before the include to read a parallel data
## root, such as the field-plot classifier variant; the default is the submission data.
BC   <- here("02.inputs", if (exists("BEETLE_ROOT")) BEETLE_ROOT else "beetle")
...
cat(sprintf("host structure: %s\n", if (USE_VRI) "annualized VRI snapshots" else "static 2025 composite"))

fmt <- function(x, d = 0) formatC(x, format = "f", digits = d, big.mark = ",")

## Journal of Pest Science wants every editable source file at submission, and the tables
## travel as one. Rather than rebuild them in a second document, which would let the two
## drift, every table is written to 03.outputs/TBL as it is printed, and Tables.qmd reads
## those files back. save_tbl returns its argument so it can sit inside a pipe.
TBLDIR <- here("03.outputs", if (exists("BEETLE_ROOT")) paste0("TBL-", BEETLE_ROOT) else "TBL")
...
## Shoulder holds 16 perimeter cells and no attack, which separates completely and returns
## a coefficient of -9.9 with a standard error of 59. It is merged into ridge, the
## neighbouring convex form, before fitting. Recorded 2026-09-02.
levels(bz$geomorphon)[levels(bz$geomorphon) == "shoulder"] <- "ridge"
...
## Never cache this chunk. knitr keys a cache on the chunk's code, not on the data it reads,
## so a cached selection outlives the maps it was made from. On 2026-09-02 the cached
## ranking from the derived-plot maps was replayed against the field-plot maps and the
## term guard stopped the render on terms the fresh selection contains.
...
## Stage four, added 2026-09-02. The three stages above filter on significance,
## collinearity and inflation, and the forest only ranks what survives, so the full model
## carried every survivor, including terms indistinguishable from zero. A lasso fit with
## the penalty chosen by ten-fold cross-validation at the one-standard-error rule removes
## the terms that do not earn their place, which is the selection the parent study used.
## One variable per pathway, the survivor the forest ranks highest, is exempt from the
## penalty so that a mechanism the Introduction names cannot be removed by shrinkage
## alone. Covariates are already standardised, so the penalty treats them alike.
X_pen <- as.matrix(bz[, VIF_KEEP])
PROTECT <- unlist(lapply(PATH, function(g) { v <- imp$term[imp$term %in% g]; if (length(v)) v[1] else NULL }))
pen_fac <- ifelse(VIF_KEEP %in% PROTECT, 0, 1)
set.seed(42)
cv_pen <- glmnet::cv.glmnet(X_pen, bz$modhigh, family = "binomial", alpha = 1,
                            penalty.factor = pen_fac, nfolds = 10, standardize = FALSE)
LAMBDA_1SE <- cv_pen$lambda.1se
pen_coef <- coef(cv_pen, s = "lambda.1se")[-1, 1]
LASSO_KEEP <- names(pen_coef)[pen_coef != 0]
FINAL <- imp$term[imp$term %in% LASSO_KEEP]
N_FINAL <- length(FINAL)
## The count the Methods report in place of the candidate count: terrain variables that
## entered the models, from the exposure, shape, landform, shading and flight-radiation
## pathways, every one of them a surface computed from the elevation model.
N_FINAL_TERRAIN <- length(intersect(FINAL, c(PATH$wind_geo, PATH$shape, PATH$landform, PATH$shading, PATH$flightsun)))

## The selection table the Methods print: every candidate, its pathway, the univariate
## screen, and the stage at which it left or the fact that it stayed.
sel_tab <- uni |>
  transmute(term, group, auc, p,
            stage = dplyr::case_when(
              !(term %in% UNI_KEEP)   ~ "univariate",
              !(term %in% CLUST_KEEP) ~ "collinearity",
              !(term %in% VIF_KEEP)   ~ "inflation",
              !(term %in% LASSO_KEEP) ~ "penalty",
              TRUE                    ~ "retained"),
            importance = imp$importance[match(term, imp$term)],
            penalised = as.numeric(pen_coef[term]),
            protected = term %in% PROTECT) |>
  arrange(factor(stage, levels = c("retained", "penalty", "inflation", "collinearity", "univariate")),
          desc(importance), desc(auc))
```
...
## M5, the annual model on the terrain-resolved MicroMet field. The Journal of Applied
## Entomology draft stopped citing it on 2026-08-31, when the annual-against-sixteen-day
## comparison came out, but the Frontiers draft still reports it and this preamble serves
## both. It is fitted here and goes unread by the draft that does not want it.
...
## what the archived static-composite draft reports. Without this switch the
## time-series draft fitted the static table and printed annualized numbers only in the
...
be$geomorphon <- factor(GEO_LAB[round(de$geomorphons)], levels = GEO_LAB)
levels(be$geomorphon)[levels(be$geomorphon) == "shoulder"] <- "ridge"
be$geomorphon <- droplevels(be$geomorphon)
...
## ---- the flight-window sensitivity table -------------------------------------------
## The sixteen-day model refitted under each definition of the wind window, on the same
## frame, covariate set and interaction as E1, so that the all-hours row IS E1 and the
## other rows differ from it only in the wind variable. Pipeline chunk 53 fitted this on
## the static-composite epoch table, which is why its all-hours row did not match the
## reported model; the fit now lives here. The wind under each definition is read from
## the rasters pipeline chunk 52 wrote, one per year and epoch.
SENS_DIR <- file.path(BC, "covariates", "wind-epoch-sensitivity")
FW_DEFS  <- c("all24", "h12_17", "h11_18", "gate")
fw_pts   <- terra::vect(as.matrix(de[, c("x", "y")]), crs = paste0("EPSG:", EPSG))
fw_fit <- function(def) {
  w <- rep(NA_real_, nrow(de))
  for (k in unique(paste(de$year, de$epoch))) {
    p <- strsplit(k, " ")[[1]]
    f <- file.path(SENS_DIR, sprintf("wind_%s_%s_e%02d.tif", def, p[1], as.integer(p[2])))
    if (!file.exists(f)) next
    idx <- de$year == as.integer(p[1]) & de$epoch == as.integer(p[2])
    w[idx] <- terra::extract(terra::rast(f), fw_pts[idx])[, 2]
  }
  keep <- !is.na(w)
  d <- de[keep, ]; d$ep_wind_mean <- w[keep]
  b <- d; for (v in EV) b[[v]] <- (b[[v]] - mean(b[[v]])) / sd(b[[v]])
  b$geomorphon <- factor(GEO_LAB[round(d$geomorphons)], levels = GEO_LAB)
  levels(b$geomorphon)[levels(b$geomorphon) == "shoulder"] <- "ridge"
  b$geomorphon <- droplevels(b$geomorphon)
  b$geomorphon <- relevel(b$geomorphon, ref = names(sort(table(b$geomorphon), decreasing = TRUE))[1])
  m <- glm(reformulate(c(EV, "geomorphon", EINT), "modhigh"), data = b, family = binomial)
  co <- as.data.frame(coef(summary(m))); names(co) <- c("beta", "se", "z", "p"); co$term <- rownames(co)
  g <- function(v, col) { x <- co[[col]][co$term == v]; if (length(x)) x else NA_real_ }
  data.frame(definition = def, n = nrow(d), wind_mean = mean(d$ep_wind_mean), wind_sd = sd(d$ep_wind_mean),
             ...
             AIC = AIC(m), AUC = as.numeric(pROC::auc(pROC::roc(b$modhigh, fitted(m), quiet = TRUE))))
}
FWS <- do.call(rbind, lapply(FW_DEFS, fw_fit))
fw  <- function(def, col) sprintf("%+.3f", FWS[[col]][FWS$definition == def])
fwp <- function(def, col) pt(FWS[[col]][FWS$definition == def])
## Guard: the all-hours row must reproduce the reported model, or the table describes a
## different fit from the one the text quotes.
fw_gap <- c(abs(FWS$stems_x_wind[FWS$definition == "all24"] - ce1$beta[ce1$term == EINT[1]]),
            abs(FWS$volume_x_wind[FWS$definition == "all24"] - ce1$beta[ce1$term == EINT[2]]))
if (any(fw_gap > 0.005)) {
  msg <- sprintf("flight-window guard: the all-hours row of the sensitivity table differs from E1 by %.4f and %.4f", fw_gap[1], fw_gap[2])
  if (isTRUE(get0("GUARD_DIAGNOSTIC", ifnotfound = FALSE))) cat("\n", msg, "\n") else stop(msg)
}
...
## Statistical reporting follows entomology journal practice, measured from 11
...
## Significance in tables is shown as asterisks beside the estimate, the convention of the
## parent study's Tables 5 and 7, with the legend in the caption. Exact p-values stay in prose.
...
if (!exists("GUARD_DIAGNOSTIC")) GUARD_DIAGNOSTIC <- FALSE
...
## On the 2025 composite that is standing volume; on annualized snapshots basal area
...
named(co3, c("solar_flight_direct", "northness", DENS[1], "QUAD_DIAM_125", "PINE_BA",
             "wind_shelter", "sky_view", "convergence", "curv_prof", "elevation",
             I_GEO, paste0(DENS[1], ":", FSUN[1])), "the full annual model M3")
...
if (!all(c("sky_view", "wind_shelter", "convergence", "curv_prof") %in% shift$term)) {
  msg <- "term-guard: the autologistic shift table lacks sky_view, wind_shelter, convergence or curv_prof"
  if (GUARD_DIAGNOSTIC) cat("\n", msg, "\n") else stop(msg)
}

## Growing-season direct radiation did not survive selection into M3, so northness
## stands in for the shading pathway. That is asserted rather than assumed, because the
## whole shading result is read off northness below.
if ("solar_season_direct" %in% co3$term) {
  msg <- "term-guard: solar_season_direct survived selection, so the shading prose, written around northness, is wrong for this fit"
  if (GUARD_DIAGNOSTIC) cat("\n", msg, "\n") else stop(msg)
}
```
