...
volume (`r pe1T(EINT[2])`), and both survived a within-season spread term carrying
...
and threshold gave `r b5(INT_MM)` (`r p5T(INT_MM)`), the opposite sign. Two further
...
of wind collapses into one of `r length(YRS)` numbers per metric, so the model has only
`r length(YRS)` distinct wind values to work with and a wind coefficient rests on those
...
`r b3("LIVE_STAND_VOLUME_125")` log-odds per standard deviation, an odds ratio of
`r or3("LIVE_STAND_VOLUME_125")`, so a stand one standard deviation above the mean in
standing wood had `r or3pc("LIVE_STAND_VOLUME_125")` per cent higher odds of
moderate-to-high disturbance (`r p3T("LIVE_STAND_VOLUME_125")`). Live stems per hectare
carried `r b3("VRI_LIVE_STEMS_PER_HA")` (`r p3T("VRI_LIVE_STEMS_PER_HA")`) and crown closure
`r b3("CROWN_CLOSURE")` (`r p3T("CROWN_CLOSURE")`), both in the opposite direction. The last two are negative, so as main effects the three
...
past the outbreak they are meant to predict. Across the `r VRI_YEARS` study years the
...
to the model, we found it cut the density by terrain-exposure interaction from `r bnr(I_GEO)` to
`r b3(I_GEO)`, without changing its sign. A landform variable in a spreading outbreak records the outbreak's own history:
...
gave `r b5(INT_MM)` at annual resolution and `r be1(EINT[1])` at sixteen days.
