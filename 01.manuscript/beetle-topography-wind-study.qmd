...
#| echo: false
#| fig-cap: "The study area over the British Columbia Freshwater Atlas, with Kootenay Lake and the Kootenay River to the east. (a) Elevation, with the 2015 Mt Midgeley burn that anchors the perimeter outlined in red. (b) Terrain ruggedness index. (c) Stand basal area from the Vegetation Resources Inventory, the density term the pheromone-disruption mechanism runs through. The study perimeter is the grey outline and contours are at 250 m. Surfaces are drawn across the whole page rather than clipped to the study area, over Esri World Shaded Relief desaturated to grey. The white outline is the study perimeter: it is the 2015 burn buffered 5 km and then cut to the 830 to 1744 m elevation band, so the ground it encloses excludes both the summit ridge, which reaches 2294 m here, and the valley floor. Contours are at 200 m. Every panel carries its own north arrow and the same numerical scale, 1:250,000, which holds at a printed panel width of 66 mm. Coordinates are EPSG:3153, NAD83(CSRS) / BC Albers."
#| fig-height: 4.6

## The unclipped display surfaces from 49-context-covariates.R, so a figure shows a
## continuous landscape rather than a ring floating on a base map. They are the same
## computations as the modelled rasters, read before the perimeter mask is applied; the
## perimeter is drawn over them so the analysed ground is still marked. dem_context is
## the elevation model over the same page.
...
academic_map(elevc, "(a) Elevation (m)", ctx, "viridis") +
...
lay <- function(r, title, pal = "viridis")
  academic_map(r, title, ctx, pal, base_size = 7, contours = TRUE)
