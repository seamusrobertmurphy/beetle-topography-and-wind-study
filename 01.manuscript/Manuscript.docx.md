...
(*Pinus contorta* Douglas ex Loudon) in British Columbia than any other cause on record
[@taylor2003; @sambaraju2021]. Mortality is not distributed evenly across a landscape, and
the stands that survive supply the structure and seed from which the next forest develops.
Such stands are termed disturbance refugia, places buffered from disturbance over time
[@krawchuk2020]. Locating a refugium is not the same as explaining one, and explanation
requires a mechanism linking survival to a measurable property of the site, such as terrain,
soil or stand structure [@cartwright2018].
...
production. Two of the three act through terrain, and none has been fitted spatially. Of
the prior work reviewed here, only @murphy2026 enters a wind term, in a companion study on
this same landscape whose response is conifer regeneration rather than beetle attack.

The wind mechanism is a claim about conditions within the canopy rather than in open air.
@cartwright2018, the only reviewed study designed to model the controls on an insect
refugium, located refugia in stands of low basal area, reasoning that "thinner stands also
increase wind penetration, helping to disperse beetle pheromones and disrupt chemical
communications needed to coordinate attacks." @powell2014 state the converse as a condition
for outbreak, that "higher local host density, which minimizes pheromone plume dispersion,
reduces wind, and promotes successful switching to nearby hosts, positively influences
outbreak propensity." A model that removes stand density and then reads a terrain
coefficient as a wind effect has eliminated the pathway it set out to test.

Terrain acts on attack through more than airflow, because flight is itself confined to a
temperature window, between 19 and 41 degrees C and mostly between 22 and 32, and most flights occur "on
bright sunny days, and peak flight is in the early to mid afternoon" [@safranyik2006chap1;
@mccambridge1971]. Radiation during the flight window is therefore a different quantity from
radiation accumulated across the season, and it is the seasonal quantity that the shading
pathway concerns. Light, temperature and wind are not competing explanations so much as a
single bundle of correlated conditions. Stand density governs "tree vigour and within-stand
microclimate, which in turn influence success of bark beetle dispersal, host selection,
attack or brood development" [@safranyik2006chap1], and @bartos1989 set the two halves of
that bundle against each other in their own title, microclimate against tree vigour. A third
route leads to the same place, because cool sites slow development enough to push the beetle
onto a two-year cycle [@sambaraju2021]. Shade and vigour therefore both predict less attack
on cool ground, by routes this design cannot separate.

Two further constraints determine the interval over which a wind term can meaningfully be
measured, and both are temporal rather than spatial. The first operates within the day.
@gray1972 recorded emergence in
...
is therefore bounded at both ends and confined to a few hours of the afternoon, so a daily
or monthly mean wind speed averages across many hours in which no beetle is flying.

The second constraint operates across the season and is a property of the population rather
than of the individual, because mass attack is a threshold phenomenon. @howe2022 conclude
...
univoltine life cycle". Two consequences follow for the present design. Attack in one year
is not independent of attack in the year before, which is why previous-year and
neighbourhood pressure enter the models. An environmental variable acting on this system
does so by regulating that threshold rather than by adding to attack, and @cooke2025 report
that "in every
...
No available product reports the quantity the hypothesis concerns, an instantaneous
below-canopy wind speed at flight height during the flight period. Gridded climatologies
report a long-run mean at 10 m over open ground and are downscaled over a digital elevation
model, which makes them in part a transform of the terrain variables offered alongside them
[@badger2014; @davis2023]. Nor does the literature specify the interval over which wind
should be summarised. Wind is therefore taken
...
@tbl-hypotheses sets out the direction expected of each attribute and the reasoning behind
it. This study addresses four questions. Does stand density predict moderate-to-high
disturbance, as
...
asymmetry it shows is the one on which the analysis depends, in that the response varies
...
band the parent study's site occupies, so that the added ground is ecologically comparable.
...
landscape, a figure not credible for a beetle outbreak. The provincial aerial overview
...
cover the mechanism and the stem-size threshold, namely total basal area, crown closure,
...
photography describes a stand the beetle had already attacked, so that basal area and pine
cover are post-attack over part of the study window. No unattacked vintage exists, and the
limitation is therefore reported rather than corrected, and returned to in the Discussion.
...
Radiation is computed twice because two mechanisms require different quantities.
Flight-window insolation is direct and diffuse radiation over 1 July to 15 August restricted
to 12:00 to 17:00, the hours identified as the flight peak [@safranyik2006chap1], while
...
One distinction should be drawn, in that the strongest aspect statements in
@safranyik2006chap1 concern the aspect of the bole rather than of the slope. These are
within-tree effects that a 30 m raster cannot measure, and nothing here tests them.
...
@liston2006. WindNinja, the tool ordinarily used for this purpose, ships no macOS binary,
whereas the wind component of MicroMet is seven equations rather than a program and was
implemented directly, with the source paper's equation numbers recorded against each.
...
The flight window is 1 July to 15 August and the hours 12:00 to 17:00, and neither bound was
chosen from these data. The dates are the flight period @safranyik2006chap1 give for this
region, and the hours follow their "peak flight is in the early to mid afternoon" together
with
...
recorded to a ceiling of 60 per cent, so that its coefficient describes a truncated variable
...
It is not restricted to the flight window, and that choice follows from how the insect
disperses. @jackson2008 tracked mountain pine beetle on weather radar and confirmed the
returns by aerial capture, finding beetles "at altitudes up to more than 800 m above the
forest canopy" and estimating that those in flight above the canopy "may move
30 to 110 km$\cdot$day$^{-1}$", at a mean density of 4,950 and a maximum of 18,600 beetles
per hectare. @ainslie2010 subsequently reproduced that above-canopy dispersion with an
atmospheric model. An insect carried in the boundary layer at those heights and over those
distances is exposed to the atmospheric regime of the period rather than to the wind
measured within a stand between noon and five, and the variable is named accordingly.
...
observed temperature fell inside the 19 to 41 degrees C gate itself. The stem-density
interaction was negative and distinguishable from zero under all four definitions, so the
...
beetle, using a response measured at the sixteen-day cadence the sensor and the insect
share, and support was partial. H2, low host density admitting the wind that disperses the
aggregation pheromone, was supported in the conditional form the mechanism specifies rather
than as a main effect. H3, a scarcity of large-diameter hosts, was supported as a threshold
rather than as a gradient. H1, topographic shading, was not supported, and the term
...
Two limits remain attached to the claim. The interaction is small, roughly a tenth of the
stand basal area main effect, and the wind field, although it varies in space, remains
terrain-modified station data rather than measurement on the ridge. What is established is
therefore that the modelled wind field behaves as the mechanism requires, not that the air
itself did.
...
the two studies were fitted on the same grid in EPSG:3153.
...
The same caution applies in reverse to every terrain term in the model, since radiation
absorbed about a third of one terrain interaction and there is no guarantee that some
further unmeasured environmental surface would not absorb the remainder, or another term
entirely.
