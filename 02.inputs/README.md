...
Province-wide, all agents, survey years 1965 to 2025. The manuscript filters server-side by extent, then to `PEST_SPECIES_CODE == "IBM"` (mountain pine beetle) and `CAPTURE_YEAR` in 1999 to 2015. That yields 301 polygons over the Darkwoods extent. Severity is the ordinal `PEST_SEVERITY_CODE`: `T` trace, `L` light, `M` moderate, `S` severe, `V` very severe.
